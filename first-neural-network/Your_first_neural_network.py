...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            delta_weights_i_h += self.lr * X[:,None].dot(hidden_error_term[:,None].T)
            # Weight step (hidden to output)
            delta_weights_h_o += self.lr * hidden_outputs[:,None].dot(output_error_term[:,None].T)
...
# 
# #### 请将你的答案填写在下方
# 1.有一定的预测效果，能大致找到每天的骑行人数峰值低谷之类的位置。
# 2.但是模型预测相对保守，每一天都给出了相似的数据，所以不是很准确。
#     可能是模型复杂度不够，使用深层网络可能效果较好。
#     可能数据还不够。
